...
void StateDefine_cdp(Ila& m) {
  ...
  NewState(m, CDP_D_OP_ENABLE, CDP_D_OP_ENABLE_BWID);

  // Square sum process bypass control and multiplier after interpolator bypass
  // control
  NewState(m, CDP_D_FUNC_BYPASS, CDP_D_FUNC_BYPASS_BWID);

  // Lower 32bits of output data address
  NewState(m, CDP_D_DST_BASE_ADDR_LOW, CDP_D_DST_BASE_ADDR_LOW_BWID);

  // Higher 32bits of output data address when axi awaddr is 64bits
  NewState(m, CDP_D_DST_BASE_ADDR_HIGH, CDP_D_DST_BASE_ADDR_HIGH_BWID);

  // Line stride of output cube
  NewState(m, CDP_D_DST_LINE_STRIDE, CDP_D_DST_LINE_STRIDE_BWID);

  // Surface stride of output cube
  NewState(m, CDP_D_DST_SURFACE_STRIDE, CDP_D_DST_SURFACE_STRIDE_BWID);

  // RAM type of output data cube
  NewState(m, CDP_D_DST_DMA_CFG, CDP_D_DST_DMA_CFG_BWID);

  // This register is not used in OpenDLA 1.0
  NewState(m, CDP_D_DST_COMPRESSION_EN, CDP_D_DST_COMPRESSION_EN_BWID);

  // Precision of input data
  NewState(m, CDP_D_DATA_FORMAT, CDP_D_DATA_FORMAT_BWID);

  // Option to flush input NaN to zero
  NewState(m, CDP_D_NAN_FLUSH_TO_ZERO, CDP_D_NAN_FLUSH_TO_ZERO_BWID);

  // Normalization length
  NewState(m, CDP_D_LRN_CFG, CDP_D_LRN_CFG_BWID);

  // Input data convertor offset
  NewState(m, CDP_D_DATIN_OFFSET, CDP_D_DATIN_OFFSET_BWID);

  // Input data convertor scale
  NewState(m, CDP_D_DATIN_SCALE, CDP_D_DATIN_SCALE_BWID);

  // Input data convertor shifter value
  NewState(m, CDP_D_DATIN_SHIFTER, CDP_D_DATIN_SHIFTER_BWID);

  // Output data convertor offset
  NewState(m, CDP_D_DATOUT_OFFSET, CDP_D_DATOUT_OFFSET_BWID);

  // Output data convertor scale
  NewState(m, CDP_D_DATOUT_SCALE, CDP_D_DATOUT_SCALE_BWID);

  // Output data convertor shifter value
  NewState(m, CDP_D_DATOUT_SHIFTER, CDP_D_DATOUT_SHIFTER_BWID);

  // input NaN element number
  NewState(m, CDP_D_NAN_INPUT_NUM, CDP_D_NAN_INPUT_NUM_BWID);

  // input Infinity element number
  NewState(m, CDP_D_INF_INPUT_NUM, CDP_D_INF_INPUT_NUM_BWID);

  // output NaN element number
  NewState(m, CDP_D_NAN_OUTPUT_NUM, CDP_D_NAN_OUTPUT_NUM_BWID);

  // saturated element number.
  NewState(m, CDP_D_OUT_SATURATION, CDP_D_OUT_SATURATION_BWID);

  // Enable/Disable performance counting
  NewState(m, CDP_D_PERF_ENABLE, CDP_D_PERF_ENABLE_BWID);

  // Element number that for both LUT under-flow
  NewState(m, CDP_D_PERF_WRITE_STALL, CDP_D_PERF_WRITE_STALL_BWID);

  // Element number that for both LUT under-flow
  NewState(m, CDP_D_PERF_LUT_UFLOW, CDP_D_PERF_LUT_UFLOW_BWID);

  // Element number that for both LUT over-flow
  NewState(m, CDP_D_PERF_LUT_OFLOW, CDP_D_PERF_LUT_OFLOW_BWID);

  // Element number that for both LUT miss, one is over-flow and the other is
  // overflow
  NewState(m, CDP_D_PERF_LUT_HYBRID, CDP_D_PERF_LUT_HYBRID_BWID);

  // Element number that for LE_lut hit only
  NewState(m, CDP_D_PERF_LUT_LE_HIT, CDP_D_PERF_LUT_LE_HIT_BWID);

  // Element number that for LO_lut hit only
  NewState(m, CDP_D_PERF_LUT_LO_HIT, CDP_D_PERF_LUT_LO_HIT_BWID);
